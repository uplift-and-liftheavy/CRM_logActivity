...
LONG_WAIT = 3
RETRY_ATTEMPTS = 5
STOP_AFTER_CLICK_BEFORE_CHECK = MED_WAIT
STOP_AFTER_CLICKED_SUCCESSFULLY = MED_WAIT
...
reportPath = os.path.join(script_dir, "Log Activity.xlsx")

if not os.path.exists(reportPath):
    raise FileNotFoundError(
        f"Log Activity workbook not found at '{reportPath}'."
    )

if not zipfile.is_zipfile(reportPath):
    raise ValueError(
        "Log Activity workbook is not a valid .xlsx file. "
        "Please re-save the file as .xlsx (not .xls or CSV)."
    )
...
try:
    ...
    raise ValueError(
        "Log Activity workbook appears to be corrupted or not a valid .xlsx file. "
        "Please re-save the file as .xlsx and try again."
    ) from exc
...
header_row = sheet1[1]
header_map = {normalize_header(cell.value): cell.column for cell in header_row}

def require_header(header_name):
    header_key = normalize_header(header_name)
    if header_key not in header_map:
        raise ValueError(f"Missing expected header: {header_name}")
    return header_map[header_key]

def ensure_header(header_name):
    header_key = normalize_header(header_name)
    if header_key in header_map:
        return header_map[header_key]
    new_col = sheet1.max_column + 1
    sheet1.cell(row=1, column=new_col).value = header_name
    header_map[header_key] = new_col
    return new_col

columnAccountID = require_header("Account ID (Not Required)")
columnContactName = require_header("Contact Name (Not Required)")
columnContactID = require_header("Contact ID (Required)")
columnDescription = require_header("Description (Required)")
columnStartDateTime = require_header("Activity Date (Required)")
columnNotes = require_header("Notes (Data required in this field)")
columnLoggedSuccessfully = ensure_header("Logged Successfully")
...
def click_element(
    ...
    stopAfterClickBeforeCheckClick=STOP_AFTER_CLICK_BEFORE_CHECK,
    stopAfterClickedSuccessfully=STOP_AFTER_CLICKED_SUCCESSFULLY
):
    """
...
def openAccountPage():
    ...
    try:
        ...
        click_element(sel_cust, "Customers Expand/Collapse", RETRY_ATTEMPTS)
        time.sleep(SHORT_WAIT)
        click_element(sel, "Account (div6) again", RETRY_ATTEMPTS)
    time.sleep(SHORT_WAIT)
...
def enterAccountNumberAndSearch(account_number):
    ...
    click_element(mag_glass_selector, "Magnifying Glass", RETRY_ATTEMPTS)
...
def process_one_row(row_index):
    ...
    click_element(open_selector, "Open Account Link", RETRY_ATTEMPTS)
    time.sleep(MED_WAIT)

    # 4a) "Open Object Detail" popup
    popup_selector = '.sapClientMQVTIOPeningButtonClass [title="Open Object Detail"]'
    click_element(popup_selector, "Open Account Popup", RETRY_ATTEMPTS)
    ...
    click_element(currentElementSelector, elementDescription, RETRY_ATTEMPTS)
    ...
    # 6a) New Visit 
    new_visit_sel = (
        '.sapClientMALPToolbarParent '
        'button.sapMBtn.width-button-form'
    )
    '''
    click_element(new_visit_sel, "New Visit", RETRY_ATTEMPTS)
    time.sleep(LONG_WAIT)
    ...
    click_element(save_sel, "Save", RETRY_ATTEMPTS)
    ...
    if check_for_element(new_visit_sel, "Confirm Save", 3):
        ...
        click_element(close_sel, "Close Account Window", RETRY_ATTEMPTS)
        time.sleep(MED_WAIT)
        click_element(close_sel, "Close Account Window", RETRY_ATTEMPTS)
        time.sleep(MED_WAIT)
        ...
        # 12c) Go Home
        home_sel = '#__div2'
        click_element(home_sel, "Home", RETRY_ATTEMPTS)
        home_check_sel = (
